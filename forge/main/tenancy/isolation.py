...
1. **RLS gate** (``__call__``): Sets ``forge.current_tenant_id`` Postgres
...
from forge.main.tenancy.rls import set_tenant_id, clear_tenant_id
...
class TenantIsolationMiddleware:
    """Set Postgres ``forge.current_tenant_id`` per request for RLS and
    ...
    def _emit_isolation_event(request, user_org, target_org_id, blocked):
        ...
        try:
            from forge.main.models.tenancy import TenantIsolationEvent
            user = getattr(request, 'user', None)
