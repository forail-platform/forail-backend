"""
Event-Driven Automation (EDA) models for Forge.
...
logger = logging.getLogger('forge.main.models.eda')
...
class EventRule(CommonModelNameNotUnique):
    ...
    def _check_generic_signature(self, body, headers):
        header_sig = headers.get('X-Forge-Signature', '')
        if not header_sig:
            raise ValueError("Missing X-Forge-Signature header")
        if '=' in header_sig:
...
class OutboundWebhook(CommonModelNameNotUnique):
    ...
    When a job/workflow reaches a matching status, Forge POSTs a signed JSON payload
    ...
    webhook_key = prevent_search(models.CharField(
        ...
        blank=True,
        default='',
        help_text=_('HMAC secret for signing outbound payloads (X-Forge-Signature header).'),
    ))
