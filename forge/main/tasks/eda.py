...
logger = logging.getLogger('forge.main.tasks.eda')


@shared_task(name='forge.main.tasks.eda.evaluate_event_rule')
def evaluate_event_rule(event_log_id):
    ...
    """
    from forge.main.models.eda import EventLog
...
def _launch_job_template(template_id, extra_vars, event_log, rule):
    ...
    merged_vars['forge_eda_event_type'] = event_log.event_type
    merged_vars['forge_eda_event_guid'] = event_log.event_guid
    merged_vars['forge_eda_rule_name'] = rule.name
    merged_vars['forge_eda_payload'] = event_log.payload
...
def _send_notification(template_id, event_log, rule):
    ...
    subject = f'[Forge EDA] Rule "{rule.name}" fired'
    body = {
        'rule_name': rule.name,
        'rule_id': rule.pk,
...
@shared_task(name='forge.main.tasks.eda.send_outbound_webhook')
def send_outbound_webhook(outbound_webhook_id, job_data):
    ...
    """
    import httpx
    from forge.main.models.eda import OutboundWebhook
    ...
    if webhook.webhook_key:
        ...
        headers['X-Forge-Signature'] = f'sha256={mac.hexdigest()}'
