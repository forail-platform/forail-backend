...
from forail.conf import models, fields
from forail.conf.settings import SettingsWrapper, EncryptedCacheProxy, SETTING_CACHE_NOTSET
from forail.conf.registry import SettingsRegistry

from forail.main.utils import encrypt_field, decrypt_field
...
def test_default_setting(settings, mocker):
    ...
    mocker.patch('forail.conf.models.Setting.objects.filter', return_value=settings_to_cache)
    assert settings.AWX_SOME_SETTING == 'DEFAULT'
    assert settings.cache.get('AWX_SOME_SETTING') == 'DEFAULT'
...
def test_empty_setting(settings, mocker):
    ...
    mocker.patch('forail.conf.models.Setting.objects.filter', return_value=mocks)
...
def test_db_setting_create(settings, mocker):
    ...
    with apply_patches(
        [
            mocker.patch('forail.conf.models.Setting.objects.filter', return_value=setting_list),
            mocker.patch('forail.conf.models.Setting.objects.create', mocker.Mock()),
        ]
    ):
...
def test_db_setting_deletion(settings, mocker):
    ...
    existing_setting = mocker.Mock(key='AWX_SOME_SETTING', value='FROM_DB')
    mocker.patch('forail.conf.models.Setting.objects.filter', return_value=[existing_setting])
...
def test_in_memory_cache_works(settings):
    ...
    with mock.patch('forail.conf.settings.SettingsWrapper._get_local', return_value='DEFAULT') as mock_get:
        assert settings.AWX_VAR == 'DEFAULT'
