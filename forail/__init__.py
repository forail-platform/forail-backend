...
try:
    ...
    __version__ = pkg_resources.get_distribution('forail').version
except Exception:
...
def prepare_env():
    ...
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'forail.settings.%s' % MODE)
...
def manage():
    ...
    # error message.  Allow --help to still work.
        ...
        sys.stdout.write('Permission denied: %s must be run as root or forail.\n' % prog)
        sys.exit(1)
    else:
        execute_from_command_line(sys.argv)
