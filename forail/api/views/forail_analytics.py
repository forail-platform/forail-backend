"""Forail Analytics views — local analytics computed from existing job data."""
...
from forail.api.generics import APIView
from forail.api.versioning import reverse
from forail.main.access import get_user_queryset
from forail.main import models

logger = logging.getLogger('forail.api.views.forail_analytics')
...
class ForailAnalyticsRootView(APIView):
    """Links to all analytics endpoints."""
    permission_classes = [IsAuthenticated]
    name = _('Forail Analytics')

    def get(self, request, format=None):
        return Response({
            'job_trends': reverse('api:forail_analytics_job_trends', request=request),
            'success_rate': reverse('api:forail_analytics_success_rate', request=request),
            'top_templates': reverse('api:forail_analytics_top_templates', request=request),
            'busiest_hosts': reverse('api:forail_analytics_busiest_hosts', request=request),
            'host_coverage': reverse('api:forail_analytics_host_coverage', request=request),
            'failure_analysis': reverse('api:forail_analytics_failure_analysis', request=request),
            'time_savings': reverse('api:forail_analytics_time_savings', request=request),
        })
