...
from forail.api.serializers.observability import ObservabilityConfigSerializer

logger = logging.getLogger('forail.api.views.observability')
...
class ObservabilityConfig(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        service_name = str(getattr(settings, 'OTEL_SERVICE_NAME', 'forail') or 'forail')
        endpoint = str(
            getattr(settings, 'OTEL_EXPORTER_ENDPOINT',
                    'http://forail-otel-collector:4317') or ''
        )
        sampler = str(
        ...
        if enabled and endpoint:
            try:
                from forail.main.observability.health import (
