...
# Copyright (c) 2026 Krstan Vjestica / Forail Project
# All Rights Reserved.
...
# AWX
from forail.main.models import (
    AdHocCommand,
    InventorySource,
    InventoryUpdate,
    Job,
...
logger = logging.getLogger('forail.api.serializers')
    ...
    def get_sub_serializer(self, obj):
        ...
        if type(self) is UnifiedJobTemplateSerializer:
            ...
            if isinstance(obj, Project):
                from forail.api.serializers import ProjectSerializer
                serializer_class = ProjectSerializer
            elif isinstance(obj, InventorySource):
                from forail.api.serializers import InventorySourceSerializer
                serializer_class = InventorySourceSerializer
            elif isinstance(obj, JobTemplate):
                from forail.api.serializers import JobTemplateSerializer
                serializer_class = JobTemplateSerializer
            elif isinstance(obj, SystemJobTemplate):
                from forail.api.serializers import SystemJobTemplateSerializer
                serializer_class = SystemJobTemplateSerializer
            elif isinstance(obj, WorkflowJobTemplate):
                from forail.api.serializers import WorkflowJobTemplateSerializer
                serializer_class = WorkflowJobTemplateSerializer
            elif isinstance(obj, WorkflowApprovalTemplate):
                from forail.api.serializers import WorkflowApprovalTemplateSerializer
...
class UnifiedJobSerializer(BaseSerializer):
    ...
    def get_sub_serializer(self, obj):
        ...
        if type(self) is UnifiedJobSerializer:
            # Lazy imports to avoid circular dependencies
            if isinstance(obj, ProjectUpdate):
                from forail.api.serializers import ProjectUpdateSerializer
                serializer_class = ProjectUpdateSerializer
            elif isinstance(obj, InventoryUpdate):
                from forail.api.serializers import InventoryUpdateSerializer
                serializer_class = InventoryUpdateSerializer
            elif isinstance(obj, Job):
                from forail.api.serializers import JobSerializer
                serializer_class = JobSerializer
            elif isinstance(obj, AdHocCommand):
                from forail.api.serializers import AdHocCommandSerializer
                serializer_class = AdHocCommandSerializer
            elif isinstance(obj, SystemJob):
                from forail.api.serializers import SystemJobSerializer
                serializer_class = SystemJobSerializer
            elif isinstance(obj, WorkflowJob):
                from forail.api.serializers import WorkflowJobSerializer
                serializer_class = WorkflowJobSerializer
            elif isinstance(obj, WorkflowApproval):
                from forail.api.serializers import WorkflowApprovalSerializer
...
class UnifiedJobListSerializer(UnifiedJobSerializer):
    ...
    def get_sub_serializer(self, obj):
        ...
        if type(self) is UnifiedJobListSerializer:
            # Lazy imports to avoid circular dependencies
            if isinstance(obj, ProjectUpdate):
                from forail.api.serializers import ProjectUpdateListSerializer
                serializer_class = ProjectUpdateListSerializer
            elif isinstance(obj, InventoryUpdate):
                from forail.api.serializers import InventoryUpdateListSerializer
                serializer_class = InventoryUpdateListSerializer
            elif isinstance(obj, Job):
                from forail.api.serializers import JobListSerializer
                serializer_class = JobListSerializer
            elif isinstance(obj, AdHocCommand):
                from forail.api.serializers import AdHocCommandListSerializer
                serializer_class = AdHocCommandListSerializer
            elif isinstance(obj, SystemJob):
                from forail.api.serializers import SystemJobListSerializer
                serializer_class = SystemJobListSerializer
            elif isinstance(obj, WorkflowJob):
                from forail.api.serializers import WorkflowJobListSerializer
                serializer_class = WorkflowJobListSerializer
            elif isinstance(obj, WorkflowApproval):
                from forail.api.serializers import WorkflowApprovalListSerializer
