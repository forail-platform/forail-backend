...
from forail.main.constants import CLOUD_PROVIDERS
from forail.main.consumers import emit_channel_notification
from forail.main.fields import (
    ImplicitRoleField,
    SmartFilterField,
    OrderedManyToManyField,
)
from forail.main.managers import HostManager, HostMetricActiveManager
from forail.main.models.base import BaseModel, CommonModelNameNotUnique, VarsDictProperty, CLOUD_INVENTORY_SOURCES, accepts_json
from forail.main.models.events import InventoryUpdateEvent, UnpartitionedInventoryUpdateEvent
from forail.main.models.unified_jobs import UnifiedJob, UnifiedJobTemplate
from forail.main.models.mixins import (
    ResourceMixin,
    TaskManagerInventoryUpdateMixin,
    RelatedJobsMixin,
    CustomVirtualEnvMixin,
)
from forail.main.models.notifications import (
    NotificationTemplate,
    JobNotificationMixin,
)
from forail.main.models.credential.injectors import _openstack_data
from forail.main.utils import _inventory_updates
from forail.main.utils.safe_yaml import sanitize_jinja
from forail.main.utils.execution_environments import to_container_path, get_control_plane_execution_environment
...
logger = logging.getLogger('forail.main.models.inventory')
...
class Inventory(CommonModelNameNotUnique, ResourceMixin, RelatedJobsMixin):
    ...
    def get_absolute_url(self, request=None):
        if request is not None:
            # circular import
            from forail.api.urls.inventory import constructed_inventory_urls
    ...
    def schedule_deletion(self, user_id=None):
        from forail.main.tasks.system import delete_inventory
        from forail.main.signals import activity_stream_delete
    ...
    def _update_host_smart_inventory_memeberships(self):
        if self.kind == 'smart' and settings.AWX_REBUILD_SMART_MEMBERSHIP:
            ...
            def on_commit():
                from forail.main.tasks.system import update_host_smart_inventory_memberships
...
class Group(CommonModelNameNotUnique, RelatedJobsMixin):
    ...
    def delete_recursive(self):
        from forail.main.utils import ignore_inventory_computed_fields
        from forail.main.tasks.system import update_inventory_computed_fields
        from forail.main.signals import disable_activity_stream, activity_stream_delete
    ...
    def job_host_summaries(self):
        from forail.main.models.jobs import JobHostSummary

        return JobHostSummary.objects.filter(host__in=self.all_hosts)

    @property
    def job_events(self):
        from forail.main.models.jobs import JobEvent

        return JobEvent.objects.filter(host__in=self.all_hosts)

    @property
    def ad_hoc_commands(self):
        from forail.main.models.ad_hoc_commands import AdHocCommand
...
class HostMetricSummaryMonthly(models.Model):
    """
    HostMetric summaries computed by scheduled task 'forail.main.tasks.system.host_metric_summary_monthly' monthly
    """
...
class InventoryUpdate(UnifiedJob, InventorySourceOptions, JobNotificationMixin, TaskManagerInventoryUpdateMixin, CustomVirtualEnvMixin):
    ...
    def _get_task_class(cls):
        from forail.main.tasks.jobs import RunInventoryUpdate
...
class PluginFileInjector(object):
    ...
    def _get_shared_env(self, inventory_update, private_data_dir, private_data_files):
        ...
        if self.base_injector == 'managed':
            from forail.main.models.credential import injectors as builtin_injectors
