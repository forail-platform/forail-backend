...
from forail.main.utils import memoize
...
class RecordedQueryLog(object):
    def __init__(self, log, db, dest='/var/log/tower/profile'):
        ...
        self.dest = dest
        try:
            self.threshold = cache.get('forail-profile-sql-threshold')
        except Exception:
    ...
    def write(self, query):
        ...
        if seconds >= self.threshold:
            ...
            version = pkg_resources.get_distribution('forail').version
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def force_debug_cursor(self):
        ...
        try:
            threshold = cache.get('forail-profile-sql-threshold')
        except Exception:
            # if we can't reach the cache, just assume profiling's off
            threshold = None
