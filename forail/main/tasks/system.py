...
# AWX
from forail import __version__ as awx_application_version
from forail.main.access import access_registry
...
from forail.main.constants import ACTIVE_STATES
from forail.main.dispatch.publish import task
from forail.main.dispatch import get_task_queuename, reaper
from forail.main.utils.common import ignore_inventory_computed_fields, ignore_inventory_group_removal

from forail.main.utils.reload import stop_local_services
from forail.main.utils.pglock import advisory_lock
from forail.main.tasks.helpers import is_run_threshold_reached
from forail.main.tasks.receptor import get_receptor_ctl, worker_info, worker_cleanup, administrative_workunit_reaper, write_receptor_config
from forail.main.consumers import emit_channel_notification
from forail.main import analytics
from forail.conf import settings_registry
from forail.main.analytics.subsystem_metrics import DispatcherMetrics

from rest_framework.exceptions import PermissionDenied

logger = logging.getLogger('forail.main.tasks.system')
...
def dispatch_startup():
    startup_logger = logging.getLogger('forail.main.tasks')
...
def profile_sql(threshold=1, minutes=1):
    if threshold <= 0:
        cache.delete('forail-profile-sql-threshold')
        logger.error('SQL PROFILING DISABLED')
    else:
        cache.set('forail-profile-sql-threshold', threshold, timeout=minutes * 60)
...
def inspect_established_receptor_connections(mesh_status):
    ...
    '''
    from forail.main.models import InstanceLink
...
def awx_k8s_reaper():
    ...
    from forail.main.scheduler.kubernetes import PodManager  # prevent circular import
...
def deep_copy_model_obj(model_module, model_name, obj_pk, new_obj_pk, user_pk, permission_check_func=None):
    ...
    from forail.api.generics import CopyAPIView
    from forail.main.signals import disable_activity_stream
