...
logger = logging.getLogger('forail.main.utils')
...
def is_testing(argv=None):
    ...
    if os.environ.get('DJANGO_SETTINGS_MODULE') == 'forail.main.tests.settings_for_test':
        return True
...
def get_awx_version():
    ...
    """
    from forail import __version__

    try:
        import pkg_resources

        return pkg_resources.require('forail')[0].version
    except Exception:
        return __version__
...
class ScheduleTaskManager(ScheduleManager):
    def __init__(self):
        from forail.main.scheduler.tasks import task_manager
...
class ScheduleDependencyManager(ScheduleManager):
    def __init__(self):
        from forail.main.scheduler.tasks import dependency_manager
...
class ScheduleWorkflowManager(ScheduleManager):
    def __init__(self):
        from forail.main.scheduler.tasks import workflow_manager
...
def truncate_stdout(stdout, size):
    from forail.main.constants import ANSI_SGR_PATTERN
...
def cleanup_new_process(func):
    ...
    def wrapper_cleanup_new_process(*args, **kwargs):
        from forail.conf.settings import SettingsWrapper  # noqa
