...
from forail.main.tasks.system import cluster_node_heartbeat
from forail.main.tasks.facts import update_hosts
...
@pytest.mark.django_db
@mock.patch('forail.main.tasks.system.inspect_execution_and_hop_nodes', lambda *args, **kwargs: None)
@mock.patch('forail.main.models.ha.get_cpu_effective_capacity', lambda cpu, is_control_node: 8)
@mock.patch('forail.main.models.ha.get_mem_effective_capacity', lambda mem, is_control_node: 62)
...
@pytest.mark.django_db
@mock.patch('forail.main.models.ha.get_cpu_effective_capacity', lambda cpu: 8)
@mock.patch('forail.main.models.ha.get_mem_effective_capacity', lambda mem: 62)
...
class TestAnsibleFactsSave:
    ...
    def test_update_hosts_forever_deadlock(self, inventory, mocker):
        ...
        for host in hosts:
            host.ansible_facts = {'foo': 'bar'}
        db_mock = mocker.patch('forail.main.tasks.facts.Host.objects.bulk_update')
    ...
    def test_update_hosts_resolved_deadlock(self, inventory, mocker):
        ...
        for host in hosts:
            host.ansible_facts = {'foo': 'bar'}
        self.current_call = 0
        mocker.patch('forail.main.tasks.facts.raw_update_hosts', new=self.fake_bulk_update)
        update_hosts(hosts)
        for host in inventory.hosts.all():
            host.refresh_from_db()
            assert host.ansible_facts == {'foo': 'bar'}
