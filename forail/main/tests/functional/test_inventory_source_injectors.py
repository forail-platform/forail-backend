...
from forail.main.models import InventorySource, Credential, CredentialType, UnifiedJob, ExecutionEnvironment
from forail.main.constants import CLOUD_PROVIDERS, STANDARD_INVENTORY_UPDATE_ENV
from forail.main.tests import data
...
def test_inventory_update_injected_content(this_kind, inventory, fake_credential_factory, mock_me):
    ...
    with mock.patch('forail.main.queue.CallbackQueueDispatcher.dispatch', lambda self, obj: None):
        ...
        with mock.patch.object(UnifiedJob, 'websocket_emit_status', mock.Mock()):
            ...
            with mock.patch('forail.main.tasks.receptor.AWXReceptorJob.run', substitute_run):
                with mock.patch('forail.main.tasks.jobs.create_partition'):
