...
# AWX models
from forail.main.models import ActivityStream, Organization, JobTemplate, Credential, CredentialType, Inventory, InventorySource, Project, User

# other AWX
from forail.main.utils import model_to_dict, model_instance_diff
...
def test_anon_user_action():
    with mock.patch('forail.main.signals.get_current_user') as u_mock:
...
def test_cluster_node_recorded(inventory, project):
    ...
    with mock.patch('forail.main.models.activity_stream.settings.CLUSTER_HOST_ID', 'foo_host'):
...
def test_cluster_node_long_node_name(inventory, project):
    jt = JobTemplate.objects.create(name='testjt', inventory=inventory, project=project)
    with mock.patch('forail.main.models.activity_stream.settings.CLUSTER_HOST_ID', 'f' * 700):
        job = jt.create_unified_job()
