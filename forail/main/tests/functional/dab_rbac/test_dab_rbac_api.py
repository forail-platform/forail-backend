...
from forail.main.models import JobTemplate, Inventory, Organization
from forail.main.access import JobTemplateAccess, WorkflowJobTemplateAccess
...
def test_custom_read_role(admin_user, post, setup_managed_roles):
    ...
    resp = post(
        url=rd_url, data={"name": "read role made for test", "content_type": "forail.inventory", "permissions": ['view_inventory']}, user=admin_user, expect=201
    )
    rd_id = resp.data['id']
