...
from forail.main.tasks.system import execution_node_health_check, _cleanup_images_and_files
from forail.main.models import Instance, Job
...
def test_does_not_run_reaped_job(mocker, mock_me):
    ...
    mock_run = mocker.patch('forail.main.tasks.jobs.ansible_runner.interface.run')
    try:
        RunJob().run(job.id)
    except Exception:
        pass
