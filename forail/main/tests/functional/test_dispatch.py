...
from forail.main.models import Job, WorkflowJob, Instance
from forail.main.dispatch import reaper
from forail.main.dispatch.pool import StatefulPoolWorker, WorkerPool, AutoscalePool
from forail.main.dispatch.publish import task
from forail.main.dispatch.worker import BaseWorker, TaskWorker
from forail.main.dispatch.periodic import Scheduler
...
def _disable_database_settings(mocker):
    m = mocker.patch('forail.conf.settings.SettingsWrapper.all_supported_settings', new_callable=mock.PropertyMock)
    m.return_value = []
...
class TestTaskDispatcher:
    ...
    def test_function_dispatch(self):
        result = self.tm.perform_work({'task': 'forail.main.tests.functional.test_dispatch.add', 'args': [2, 2]})
        assert result == 4

    def test_function_dispatch_must_be_decorated(self):
        result = self.tm.perform_work({'task': 'forail.main.tests.functional.test_dispatch.restricted', 'args': [2, 2]})
        assert isinstance(result, ValueError)
        assert str(result) == 'forail.main.tests.functional.test_dispatch.restricted is not decorated with @task()'  # noqa

    def test_method_dispatch(self):
        result = self.tm.perform_work({'task': 'forail.main.tests.functional.test_dispatch.Adder', 'args': [2, 2]})
        assert result == 4
    ...
    def test_undefined_function_cannot_be_imported(self):
        result = self.tm.perform_work({'task': 'forail.foo.bar'})
        assert isinstance(result, ModuleNotFoundError)
        assert str(result) == "No module named 'forail.foo'"  # noqa
...
class TestTaskPublisher:
    ...
    def test_function_apply_async(self):
        ...
        assert message['task'] == 'forail.main.tests.functional.test_dispatch.add'
        assert queue == 'foobar'
    ...
    def test_method_apply_async(self):
        ...
        assert message['args'] == [2, 2]
        assert message['kwargs'] == {}
        assert message['task'] == 'forail.main.tests.functional.test_dispatch.Adder'
        assert queue == 'foobar'

    def test_apply_async_queue_required(self):
        with pytest.raises(ValueError) as e:
            message, queue = add.apply_async([2, 2])
        assert "forail.main.tests.functional.test_dispatch.add: Queue value required and may not be None" == e.value.args[0]
...
class TestScheduler:
    ...
    def test_missed_schedule(self, mocker):
        ...
        mocker.patch('forail.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 50)
    ...
    def test_advance_schedule(self, mocker):
        ...
        for job in scheduler.jobs:
            ...
            job.offset = 0.0
        mocker.patch('forail.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 29)
        to_run = scheduler.get_and_mark_pending()
        assert set(job.name for job in to_run) == set(['joba', 'jobb'])
        mocker.patch('forail.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 39)
        to_run = scheduler.get_and_mark_pending()
        assert len(to_run) == 1
    ...
    def test_scheduler_debug(self, mocker):
        ...
        mocker.patch('forail.main.dispatch.periodic.time.time', return_value=current_time - 1.0e-8)
