...
from forail.api.serializers import JobLaunchSerializer
from forail.main.models import Credential, Inventory, Host, ExecutionEnvironment, Label, InstanceGroup
from forail.main.models.jobs import Job, JobTemplate, UnifiedJobTemplate
...
def test_job_ignore_unprompted_vars(runtime_data, job_template_prompts, post, admin_user, mocker):
    ...
    mocker.patch('forail.api.serializers.JobSerializer.to_representation')
...
def test_job_launch_unprompted_vars_with_survey(mocker, survey_spec_factory, job_template_prompts, post, admin_user):
    ...
    mocker.patch('forail.main.access.BaseAccess.check_license')
    mock_job = mocker.MagicMock(spec=Job, id=968, extra_vars={"job_launch_var": 3, "survey_var": 4})
    mocker.patch.object(JobTemplate, 'create_unified_job', return_value=mock_job)
    mocker.patch('forail.api.serializers.JobSerializer.to_representation', return_value={})
    response = post(
        reverse('api:job_template_launch', kwargs={'pk': job_template.pk}),
...
def test_callback_accept_prompted_extra_var(mocker, survey_spec_factory, job_template_prompts, post, admin_user, host):
    ...
    mocker.patch('forail.api.serializers.JobSerializer.to_representation', return_value={})
    mocker.patch('forail.api.views.JobTemplateCallback.find_matching_hosts', return_value=[host])
    post(
