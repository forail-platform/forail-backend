...
from forail.main.models.credential import HIDDEN_PASSWORD, ManagedCredentialType

from forail.main.tasks import jobs, system, receptor
from forail.main.utils import encrypt_field, encrypt_value
from forail.main.utils.safe_yaml import SafeLoader
from forail.main.utils.execution_environments import CONTAINER_ROOT

from forail.main.utils.licensing import Licenser
from forail.main.constants import JOB_VARIABLE_PREFIXES
...
def mock_create_partition():
    with mock.patch('forail.main.tasks.jobs.create_partition') as cp_mock:
        yield cp_mock
...
def test_send_notifications_job_id(mocker):
    mocker.patch('forail.main.models.UnifiedJob.objects.get')
    system.send_notifications([], job_id=1)
    assert UnifiedJob.objects.get.called
    UnifiedJob.objects.get.assert_called_with(id=1)


@mock.patch('forail.main.models.UnifiedJob.objects.get')
@mock.patch('forail.main.models.Notification.objects.filter')
...
class TestGenericRun:
    def test_generic_failure(self, patch_Job, execution_environment, mock_me, mock_create_partition):
        ...
        with mock.patch('forail.main.tasks.jobs.shutil.copytree'):
    ...
    def test_awx_task_env(self, patch_Job, private_data_dir, execution_environment, mock_me):
        ...
        with mock.patch('forail.main.tasks.jobs.settings.AWX_TASK_ENV', {'FOO': 'BAR'}):
...
def test_administrative_workunit_reaper(work_unit_data, expected_function_call):
    ...
    with mock.patch('forail.main.tasks.receptor.get_receptor_ctl') as mock_get_receptor_ctl:
