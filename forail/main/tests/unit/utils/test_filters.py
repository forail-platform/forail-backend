...
# AWX
from forail.main.utils.filters import SmartFilter, ExternalLoggerEnabled
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'enabled_flag': False}, 'forail.main', False),
        # skip all records if the host is undefined
        ({'enabled_flag': True}, 'forail.main', False),
        # skip all records if underlying logger is used by handlers themselves
        ({'enabled_flag': True}, 'forail.main.utils.handlers', False),
        ({'enabled_flag': True, 'enabled_loggers': ['forail']}, 'forail.main', True),
        ({'enabled_flag': True, 'enabled_loggers': ['abc']}, 'forail.analytics.xyz', False),
        ({'enabled_flag': True, 'enabled_loggers': ['xyz']}, 'forail.analytics.xyz', True),
    ],
)
...
@mock.patch('forail.main.utils.filters.get_model', return_value=mockHost())
