...
pas_inputs = {
    'fields': [
        ...
        },
        {
            ...
            'type': 'string',
            'help_text': _('Application ID of the configured OAuth2 Client (defaults to \'forail\')'),
            'default': 'forail',
        },
        {
            'id': 'oauth_scope',
            ...
            'type': 'string',
            'help_text': _('Scope of the configured OAuth2 Client (defaults to \'forail\')'),
            'default': 'forail',
        },
    ],
    'metadata': [
...
def centrify_backend(**kwargs):
    ...
    app_id = kwargs.get('oauth_application_id', 'forail')
    endpoint = urljoin(url, f'/oauth2/token/{app_id}')
    endpoint = {'endpoint': endpoint, 'client_id': client_id, 'client_password': client_password, 'oauth_scope': kwargs.get('oauth_scope', 'forail')}
